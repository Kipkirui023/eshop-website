<?php
    ...
    if (isset($_POST['add_product'])){
        ...
        $select_product_name = mysqli_query($conn, "SELECT name FROM `products` WHERE name = '$product_name'") or die('Query Fails');
        if (mysqli_num_rows($select_product_name)>0){
            $message[] = 'Product Name Already Exist';
        }else{
            $insert_product = mysqli_query($conn, "INSERT INTO `products` (`name`, `price`, `product_detail`,`image`) VALUES('$product_name', '$product_price', '$product_detail', '$image')") or die('Query Fails');
            if($insert_product){
    ...
    if (isset($_GET['delete'])){
        ...
        $select_delete_image = mysqli_query($conn, "SELECT image FROM `products` WHERE id = '$delete_id'") or die('Query Fails');
        $fetch_delete_image = mysqli_fetch_assoc($select_delete_image);
        unlink('image/'.$fetch_delete_image['image']);

        mysqli_query($conn, "DELETE FROM `products` WHERE id = '$delete_id'") or die('Query Fails');
        mysqli_query($conn, "DELETE FROM `cart` WHERE pid = '$delete_id'") or die('Query Fails');
        mysqli_query($conn, "DELETE FROM `wishlist` WHERE pid = '$delete_id'") or die('Query Fails');
    ...
    if (isset($_POST['update-product'])){
        ...
        `product_detail` = '$update_detail', `image` = '$update_image' WHERE `id` ='$update_id'") or die('Update Query Fails');
        if($update_query){
...
<body>
    ...
    <section class="show-products">
        <div class="box-container">
            ...
            <div class="box">
                ...
                <p>Price : Khs. <?php echo $fetch_products['price'];?></p>
    ...
    <section class="update-container">
        ...
        if (isset($_GET['edit'])){
            $edit_id = $_GET['edit'];
            $edit_query = mysqli_query($conn, "SELECT *FROM `products` WHERE id = '$edit_id'") or die('Query Fails');
